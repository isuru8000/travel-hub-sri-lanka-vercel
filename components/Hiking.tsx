...
const Hiking: React.FC<HikingProps> = ({ language, setView }) => {
  return (
    <section className="min-h-screen bg-white pb-32">
      ...
      {/* Main Exploration Grid - Updated to 4 columns on XL */}
      <div className="max-w-[1600px] mx-auto px-6 md:px-8 -mt-24 relative z-10 space-y-32">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 md:gap-10">
          {HIKING_DATA.map((spot, idx) => (
            <div 
              key={spot.id}
              className="group relative bg-white rounded-[4rem] overflow-hidden border border-gray-100 shadow-[0_20px_60px_rgba(0,0,0,0.05)] transition-all duration-1000 hover:shadow-[0_80px_150px_rgba(0,0,0,0.12)] hover:-translate-y-6 flex flex-col"
            >
              <div className="relative h-[400px] overflow-hidden">
                ...
                <div className="absolute top-8 left-8 flex flex-col gap-4 transform translateZ(50px)">
                  <div className="px-5 py-2.5 bg-white/10 backdrop-blur-3xl rounded-2xl border border-white/20 text-white text-[8px] font-black uppercase tracking-widest flex items-center gap-2 shadow-2xl">
                     <TrendingUp size={12} className="text-[#E1306C]" />
                     Ref: #H0{idx + 1}
                  </div>
                  <div className={`px-5 py-2.5 rounded-2xl border border-white/20 text-white text-[8px] font-black uppercase tracking-widest flex items-center gap-2 shadow-2xl backdrop-blur-3xl ${spot.difficulty === 'Hard' ? 'bg-red-500/30' : spot.difficulty === 'Moderate' ? 'bg-orange-500/30' : 'bg-green-500/30'}`}>
                ...
                <div className="absolute bottom-8 left-8 right-8 space-y-3">
                   <div className="flex items-center gap-2 text-[#E1306C] drop-shadow-lg">
                      <MapPin size={14} />
                      <span className="text-[10px] font-black uppercase tracking-[0.3em]">{spot.location[language]}</span>
                   </div>
                   <h3 className="text-2xl md:text-3xl font-heritage font-bold text-white uppercase tracking-tight group-hover:insta-text-gradient transition-all leading-none">
                      {spot.name[language]}
                   </h3>
                </div>
              </div>
              ...
              <div className="p-10 space-y-8 flex-grow flex flex-col">
                <p className="text-lg text-gray-500 font-light italic leading-relaxed border-l-2 border-gray-50 pl-6 line-clamp-3">
                   "{spot.description[language]}"
                </p>

                <div className="grid grid-cols-2 gap-6 border-y border-gray-50 py-8">
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center text-[#E1306C] shadow-inner group-hover:bg-[#E1306C] group-hover:text-white transition-all duration-500">
                         <Clock size={20} />
                      </div>
                      <div className="space-y-0.5">
                         <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest">Duration</p>
                         <p className="text-sm font-bold text-[#0a0a0a]">{spot.duration[language]}</p>
                      </div>
                   </div>
                   <div className="flex items-center gap-4">
                      <div className="w-12 h-12 rounded-xl bg-gray-50 flex items-center justify-center text-[#E1306C] shadow-inner group-hover:bg-[#E1306C] group-hover:text-white transition-all duration-500">
                      ...
                      <div className="space-y-0.5">
                         <p className="text-[9px] font-black text-gray-300 uppercase tracking-widest">Summit</p>
                         <p className="text-sm font-bold text-[#0a0a0a]">{spot.elevation}</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 mt-auto">
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-[8px] font-black text-[#0a0a0a] uppercase tracking-widest leading-none">Synced</span>
                   </div>
                   <button className="flex items-center gap-4 px-6 py-4 bg-[#0a0a0a] text-white rounded-2xl font-black uppercase tracking-[0.3em] text-[9px] transition-all hover:bg-[#E1306C] shadow-xl group/btn">
                      Begin Expedition
